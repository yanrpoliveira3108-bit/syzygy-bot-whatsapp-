...
// Adaptador da Payment Message REAL do Baileys instalado:
//   @innovatorssoft/baileys 7.4.7
//   sock.sendMessage(jid, { payment: PaymentInfo })
//   → proto requestPaymentMessage { amount1000, currencyCodeIso4217, noteMessage, requestFrom }
//
// paymentInvite existe no fork, mas NÃO carrega valor/moeda — não é o payload de pagamento.
// Não inventar API. Se sendMessage/payment não estiver disponível em runtime, o adaptador recusa.
...
export async function sendPaymentMessage(sock, jid, payload, extra = {}) {
    if (!sock || typeof sock.sendMessage !== "function") {
        throw Object.assign(new Error("PAYMENT_UNAVAILABLE"), { code: "PAYMENT_UNAVAILABLE" })
    }
    const from = extra.from || sock.user?.id
    const content = buildPaymentContent(payload, { from, mentions: extra.mentions, expiry: extra.expiry })
